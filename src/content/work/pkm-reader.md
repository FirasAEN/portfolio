...
outline. I run it for my own notes.

## Ignore rules turned out to be the whole job
...
never opens `dist/` to discover the exception. So reproducing git faithfully
means reproducing a limitation, which felt wrong for about a day.

Then I decided it was not. A tool that is *almost* compatible with a familiar
format is worse than one that is exactly compatible, because the gap only ever
shows up as surprise: someone reasons from what git would do, gets something
else, and the mechanism they got it wrong about is the one that decides what
becomes public. I would rather ship the limitation.
...
That is in the README along with the others: no `extends` chain following in
tsconfig, no support for git's global excludes file, package-style Python and Go
imports that never resolve to links. It is the part of the README I would keep if
I had to cut the rest.

Access is gated by signed-cookie middleware at the edge, with one bypass: a
